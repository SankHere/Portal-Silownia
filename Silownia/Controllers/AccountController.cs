        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    Profile profile = new Profile { UserName = model.Email, DayOfPass = 0, Name = model.Name, Surname = model.Surname, City = model.City, NumberPhone = model.Number};
                    GymContext db = new GymContext();
                    db.Profiles.Add(profile);
                    db.SaveChanges();
                    SendMail(model.Email, "Rejestracja", "Rejestracja udana, możesz się zalogować");
